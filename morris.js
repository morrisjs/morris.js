...
//            return this.raphael.path("M40,20L60,50").attr('stroke', lineColor).attr('stroke-width', this.options.lineWidth)
//                .animate({path: path},2000);
//          draw animated Line
            split_path = split_line_path(path)
            var complete_path, start_pos, new_path = ""
            for (i = _i = 0, _len = split_path.length; _i < _len; i = ++_i) {
                if(i > 0){
                    last_path_split = split_path[i-1].split(",")
                    path_split = split_path[i].split(",")
                    len = last_path_split.length

                    xM = last_path_split[len-2].replace("M","");
                    yM = last_path_split[len-1];
                    start_pos = "M" + xM +","+ yM + " L " + path_split[0] +","+path_split[1];
                    new_path = "M" + xM +","+ yM + "C" +split_path[i]
                }
                complete_path = this.raphael.path(start_pos).attr('stroke', lineColor).attr('stroke-width', this.options.lineWidth).animate({path:new_path},1000)
            }
        };

        function split_line_path(path){
            arr = path.split("C");
            first_arr = arr[0]+"C"+arr[1]
            return arr
        }
...
//            var test =
            return this.raphael.rect(xPos,yPos+height, width, 0).attr('fill', barColor).attr('stroke-width', 0)
                .animate({y:yPos,height:height}, 1000);
        };
